...
export class ProductsService {
  ...
  async create(createProductDto: CreateProductDto, imageUrl?: string) {
    const slug = createProductDto.name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)+/g, '');
    ...
    const [product] = await this.db.insert(schema.products).values(newProduct).returning();
    return product;
  }
  ...
  async findAll(query?: any) {
    ...
    if (query?.category && query.category !== 'All') {
      conditions.push(eq(schema.products.category, query.category));
    }
