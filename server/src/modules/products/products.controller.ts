import { Controller, Post, Get, Put, Delete, Param, Req, Res, Query } from '@nestjs/common';
...
export class ProductsController {
  ...
  async create(@Req() req: FastifyRequest, @Res() res: FastifyReply) {
    if (!req.isMultipart()) {
      return res.status(400).send({ success: false, message: 'Request is not multipart' });
    }
    ...
    if (!name || isNaN(price) || isNaN(stock)) {
      return res.status(400).send({ success: false, message: 'Missing required fields or invalid types' });
    }

    const createProductDto = { name, price, stock, description, category };
  ...
  @Put(':id')
  async update(@Param('id') id: string, @Req() req: FastifyRequest, @Res() res: FastifyReply) {
    if (!req.isMultipart()) {
      return res.status(400).send({ success: false, message: 'Request is not multipart' });
    }
    ...
    const updateDto = { name, price, stock, description, category };
    const updatedProduct = await this.productsService.update(id, updateDto, imageUrl);
